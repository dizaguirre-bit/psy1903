...
summarize_behavior <- function(data, rt_min = 300, rt_max = 900) {
  ...
  # valid_data_rt <- data[data[] >= 300 & data[ , 3] <= 900 & data[ , 12] == TRUE, ]
  # valid_data_acc <- data[data[ , 3] >= 300 & data[ , 3] <= 900, ]
  
  ## Refactor #1: fix indexing with column names for clarity, adjust threshholds for flexibility 
  ## of use by the function
  ...
  ## Refactor #2: Remove this centralization of participant RT as it is never used again 
  ...
  ## FIX: Column name fixed to expectedCategoryAsDisplayed, instead of expectedCategoryDisplayed as it was originally
