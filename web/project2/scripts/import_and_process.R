...
import_and_process <- function(file_name) {
  ...
  ## FIX 2: replace incorrect column names found in raw data with correct names
  
  if ("trial_type" %in% names(df) &&
      !"trialType" %in% names(df)) {
  ...
  if ("expectedCategoryDisplayed" %in% names(df) &&
      !"expectedCategoryAsDisplayed" %in% names(df)) {
    
    warning(
      paste(
        "Column 'expectedCategoryDisplayed' found in",
        sub("\\.csv$", "", basename(file_name)),
        "-- renaming to 'expectedCategoryAsDisplayed'."
      )
    )
  ...
  ## FIX #1: In making questionnaire_df, "Questionnaire" was capitalized to properly match its value in TrialType
